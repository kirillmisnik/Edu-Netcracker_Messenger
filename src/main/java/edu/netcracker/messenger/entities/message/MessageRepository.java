package edu.netcracker.messenger.entities.message;
...
public interface MessageRepository extends JpaRepository<Message, Long> {
     ...
     * @return list of messages
     */
    @Query(value = "SELECT * FROM messages WHERE creation_date <= (SELECT creation_date FROM messages WHERE message_id = ?1) AND chat_id = (SELECT chat_id FROM messages WHERE message_id = ?1) ORDER BY creation_date", nativeQuery = true)
